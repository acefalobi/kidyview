...
public class CameraSender extends AttachmentSender {
    ...
    private void startCameraIntent(Activity activity) {
        ...
        openCamera(activity, file, ACTIVITY_REQUEST_CODE);
    }

    protected void openCamera(Activity activity, File file, int requestCode) {
    ...
    @NonNull protected File getTempFile(Activity activity) {
    ...
    public void sendMessage(Activity activity) {
        ...
        try {
            ...
            File file = new File(mPhotoFilePath.get());
            if (file.exists()) {
                Message message = ThreePartImageUtils.newThreePartImageMessage(activity, getLayerClient(), file);
            ...
            mPhotoFilePath.set(null);
        }
    }

    public boolean photoPathIsEmpty() {
        return mPhotoFilePath.get() == null
                || !new File(mPhotoFilePath.get()).exists();
    }
