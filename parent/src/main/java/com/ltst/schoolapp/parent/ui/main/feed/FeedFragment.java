...
import com.ltst.schoolapp.parent.ui.dated.feed.DatedFeedScope;
...
public class FeedFragment extends CoreFragment implements FeedContract.View {
    ...
    protected void onCreateComponent(HasSubComponents hasSubComponents) {
        if (hasSubComponents instanceof MainActivity) {
            isMain = true;
            MainScope.MainComponent component = (MainScope.MainComponent) hasSubComponents.getComponent();
            component.feedComponent(new FeedScope.FeedModule(this, isMain)).inject(this);
        } else {
            isMain = false;
            DatedFeedScope.DatedFeedComponent component = (DatedFeedScope.DatedFeedComponent) hasSubComponents.getComponent();
            component.feedComponent(new FeedScope.FeedModule(this, isMain)).inject(this);
        }
    }
...
//        toolbarSpinner.setVisibility(View.VISIBLE);
    ...
    public void initToolbar(int navigationIcon, View.OnClickListener onNavigationClick,
                            Toolbar.OnMenuItemClickListener listener, boolean showSpinner, String title) {
        if (navigationIcon != 0)
            toolbar.setNavigationIcon(navigationIcon);
        toolbar.setNavigationOnClickListener(onNavigationClick);
        toolbar.setOnMenuItemClickListener(listener);
        if (toolbarSpinner != null) {
            toolbarSpinner.setVisibility(showSpinner ? View.VISIBLE : View.GONE);
        }
        if (title != null)
...
//        toolbarSpinner.setOnItemSelectedListener(spinnerListener);
...
//    @Override public void bindSpinnerData(SpinnerAdapter spinnerAdapter, SimpleItemSelectedListener spinnerListener) {
//        toolbarSpinner.setAdapter(spinnerAdapter);
//        toolbarSpinner.setOnItemSelectedListener(spinnerListener);
//    }
